...
def assign_wci_to_examples(df, text_column='text'):
    df['wci'] = df[text_column].apply(calculate_word_count)
    return df


def calculate_average_wci(df):
    average_wci = df['wci'].mean()
    return average_wci


def calculate_dataset_wci(file_path, text_column='text'):
...
def redistribute_wci(df, sample_size):
    min_count = df['shared_emotion'].value_counts().min() // 3
    sample_size = min(sample_size, min_count)

    high_wci_df = pd.DataFrame(columns=df.columns)
    mid_wci_df = pd.DataFrame(columns=df.columns)
    low_wci_df = pd.DataFrame(columns=df.columns)

    for emotion in df['shared_emotion'].unique():
        subset = df[df['shared_emotion'] == emotion].copy()
        subset = subset.sort_values('wci', ascending=False)

        high_third = subset.iloc[:len(subset) // 3, :]
        mid_third = subset.iloc[len(subset) // 3:2 * len(subset) // 3, :]
        low_third = subset.iloc[2 * len(subset) // 3:, :]

        high_wci_df = high_wci_df.append(high_third.sample(sample_size))
        mid_wci_df = mid_wci_df.append(mid_third.sample(sample_size))
        low_wci_df = low_wci_df.append(low_third.sample(sample_size))

    return high_wci_df, mid_wci_df, low_wci_df
...
def create_and_save_datasets(directory, iterations, df, sample_size):
    ...
    for i in tqdm(range(1, iterations + 1)):
        high_df, mid_df, low_df = redistribute_wci(df.copy(), sample_size)

        high_df.to_csv(os.path.join(directory, 'high', f'high_wci_{i}.csv'), index=False)
        mid_df.to_csv(os.path.join(directory, 'mid', f'mid_wci_{i}.csv'), index=False)
        low_df.to_csv(os.path.join(directory, 'low', f'low_wci_{i}.csv'), index=False)
...
def create_summary_csv(directory, wci_level, iterations):
    results = []
    for i in range(1, iterations + 1):
        filename = os.path.join(directory, wci_level, f'{wci_level}_wci_{i}.csv')
    ...
    summary_df.to_csv(os.path.join(directory, f'{wci_level}_results.csv'), index=False)
...
if RECALCULATE_DATASETS:
    wci_dataframe = assign_wci_to_examples(train_data)
    create_and_save_datasets(WORK_DIR, 5, wci_dataframe, 10000)
...
class EmotionModel:
    ...
    def _stratify_and_subset(self, df, test_size=0.05):
        ...
        df['shared_emotion_encoded'] = self.label_encoder.fit_transform(df['shared_emotion'])
...
file_dict = {
    'high_wci_1.csv': os.path.join(WORK_DIR, 'high', 'high_wci_1.csv'),
    'high_wci_2.csv': os.path.join(WORK_DIR, 'high', 'high_wci_2.csv'),
    'high_wci_3.csv': os.path.join(WORK_DIR, 'high', 'high_wci_3.csv'),
    'high_wci_4.csv': os.path.join(WORK_DIR, 'high', 'high_wci_4.csv'),
    'high_wci_5.csv': os.path.join(WORK_DIR, 'high', 'high_wci_5.csv'),
    'low_wci_1.csv': os.path.join(WORK_DIR, 'low', 'low_wci_1.csv'),
    'low_wci_2.csv': os.path.join(WORK_DIR, 'low', 'low_wci_2.csv'),
    'low_wci_3.csv': os.path.join(WORK_DIR, 'low', 'low_wci_3.csv'),
    'low_wci_4.csv': os.path.join(WORK_DIR, 'low', 'low_wci_4.csv'),
    'low_wci_5.csv': os.path.join(WORK_DIR, 'low', 'low_wci_5.csv'),
    'mid_wci_1.csv': os.path.join(WORK_DIR, 'mid', 'mid_wci_1.csv'),
    'mid_wci_2.csv': os.path.join(WORK_DIR, 'mid', 'mid_wci_2.csv'),
    'mid_wci_3.csv': os.path.join(WORK_DIR, 'mid', 'mid_wci_3.csv'),
    'mid_wci_4.csv': os.path.join(WORK_DIR, 'mid', 'mid_wci_4.csv'),
    'mid_wci_5.csv': os.path.join(WORK_DIR, 'mid', 'mid_wci_5.csv'),
}
...
for filename, file_path in file_dict.items():
    overall_wci = calculate_dataset_wci(file_path)
    em = EmotionModel()
    model, stats = em.train_model(file_path, test=testing, num_classes=11)
    em.stats['overall_wci'] = overall_wci
    all_trained_models[filename] = em

with open(os.path.join(WORK_DIR, 'wci_all_trained_models.pkl'), 'wb') as f:
    pickle.dump(all_trained_models, f)

with open(os.path.join(WORK_DIR, 'wci_all_trained_models.pkl'), 'rb') as f:
    all_trained_models = pickle.load(f)
...
results_df.to_csv(os.path.join(WORK_DIR, 'wci_all_models_final_results.csv'), index=False)
...
with open(os.path.join(WORK_DIR, 'BLSTM_e_wci_all_trained_models.pkl'), 'wb') as f:
...
results_df.to_csv(os.path.join(WORK_DIR, 'BLSTM_e_wci_all_models_final_results.csv'), index=False)
