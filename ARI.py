...
def assign_ari_to_examples(df, text_column='text'):
    df['ari'] = df[text_column].apply(calculate_ari)
    return df


def calculate_average_ari(df):
    average_ari = df['ari'].mean()
    return average_ari


def calculate_dataset_ari(file_path, text_column='text'):
...
def redistribute_ari(df, sample_size):
    min_count = df['shared_emotion'].value_counts().min() // 3
    sample_size = min(sample_size, min_count)

    high_ari_df = pd.DataFrame(columns=df.columns)
    mid_ari_df = pd.DataFrame(columns=df.columns)
    low_ari_df = pd.DataFrame(columns=df.columns)

    for emotion in df['shared_emotion'].unique():
        subset = df[df['shared_emotion'] == emotion].copy()
        subset = subset.sort_values('ari', ascending=False)

        high_third = subset.iloc[:len(subset) // 3, :]
        mid_third = subset.iloc[len(subset) // 3:2 * len(subset) // 3, :]
        low_third = subset.iloc[2 * len(subset) // 3:, :]

        high_ari_df = high_ari_df.append(high_third.sample(sample_size))
        mid_ari_df = mid_ari_df.append(mid_third.sample(sample_size))
        low_ari_df = low_ari_df.append(low_third.sample(sample_size))

    return high_ari_df, mid_ari_df, low_ari_df
...
def create_summary_csv(directory, ari_level, iterations):
    results = []
    for i in range(1, iterations + 1):
        filename = os.path.join(directory, ari_level, f'{ari_level}_ari_{i}.csv')
    ...
    summary_df.to_csv(os.path.join(directory, f'{ari_level}_results.csv'), index=False)
...
ari_dataframe = assign_ari_to_examples(train_data)
if RECALCULATE_DATASETS:
    create_and_save_datasets(WORK_DIR, 5, ari_dataframe, 10000)
...
file_dict = {
    'high_ari_1.csv': os.path.join(WORK_DIR, 'high', 'high_ari_1.csv'),
    'high_ari_2.csv': os.path.join(WORK_DIR, 'high', 'high_ari_2.csv'),
    'high_ari_3.csv': os.path.join(WORK_DIR, 'high', 'high_ari_3.csv'),
    'high_ari_4.csv': os.path.join(WORK_DIR, 'high', 'high_ari_4.csv'),
    'high_ari_5.csv': os.path.join(WORK_DIR, 'high', 'high_ari_5.csv'),
    'mid_ari_1.csv': os.path.join(WORK_DIR, 'mid', 'mid_ari_1.csv'),
    'mid_ari_2.csv': os.path.join(WORK_DIR, 'mid', 'mid_ari_2.csv'),
    'mid_ari_3.csv': os.path.join(WORK_DIR, 'mid', 'mid_ari_3.csv'),
    'mid_ari_4.csv': os.path.join(WORK_DIR, 'mid', 'mid_ari_4.csv'),
    'mid_ari_5.csv': os.path.join(WORK_DIR, 'mid', 'mid_ari_5.csv'),
    'low_ari_1.csv': os.path.join(WORK_DIR, 'low', 'low_ari_1.csv'),
    'low_ari_2.csv': os.path.join(WORK_DIR, 'low', 'low_ari_2.csv'),
    'low_ari_3.csv': os.path.join(WORK_DIR, 'low', 'low_ari_3.csv'),
    'low_ari_4.csv': os.path.join(WORK_DIR, 'low', 'low_ari_4.csv'),
    'low_ari_5.csv': os.path.join(WORK_DIR, 'low', 'low_ari_5.csv'),
}
...
for filename, file_path in file_dict.items():
    overall_ari = calculate_dataset_ari(file_path)
    em = EmotionModel()
    model, stats = em.train_model(file_path, test=testing, num_classes=11)
    em.stats['overall_ari'] = overall_ari
    all_trained_models[filename] = em

with open(os.path.join(WORK_DIR, 'ari_all_trained_models.pkl'), 'wb') as f:
    pickle.dump(all_trained_models, f)

with open(os.path.join(WORK_DIR, 'ari_all_trained_models.pkl'), 'rb') as f:
...
results_df.to_csv(os.path.join(WORK_DIR, 'ari_all_models_final_results.csv'), index=False)
...
class NN_EmotionModel:
    ...
    def train_model(self, path, tag, test=False, embedding_dim=64, hidden_dim=32, epochs=10, batch_size=1024,
        ...
        with torch.no_grad():
            for sentence, labels in test_loader:
                sentence_lengths = torch.clamp(sentence.ne(0).sum(1), min=1,
                                               max=self.max_seq_length)
                tag_scores = self.model(sentence, sentence_lengths)
    ...
    def predict_emotion(self, text):
        ...
        self.model.eval()
        with torch.no_grad():
            inputs = [self.word_to_ix.get(word, 0) for word in
                      text.split()]
...
with open(os.path.join(WORK_DIR, 'BLSTM_c_ari_all_trained_models.pkl'), 'wb') as f:
...
results_df.to_csv(os.path.join(WORK_DIR, 'BLSTM_c_ari_all_models_final_results.csv'), index=False)
